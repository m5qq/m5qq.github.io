document.addEventListener("DOMContentLoaded", () => {
    ...
    function requestFullscreenCrossBrowser(element) {
    ...
    if (videoElement && coverElement) {
        const hideCover = () => coverElement.classList.add('is-hidden');
        const showCover = () => coverElement.classList.remove('is-hidden');

        const enterFullscreenAndPlay = async () => {
            try {
                await requestFullscreenCrossBrowser(videoElement);
            } catch (e) {
        ...
        coverElement.addEventListener('click', enterFullscreenAndPlay);
        videoElement.addEventListener('click', async () => {
            if (!isFullscreenActive()) {
                await enterFullscreenAndPlay();
            }
        });

        const onExitFullscreen = () => {
            if (!isFullscreenActive()) {
                // Exit FS: pause, show cover, keep video at container size
        ...
        videoElement.addEventListener('play', hideCover);
        videoElement.addEventListener('pause', () => {
            if (videoElement.currentTime === 0 || videoElement.ended) {
                showCover();
            }
        });
        videoElement.addEventListener('ended', showCover);
    }
});
